...
public class ExploreBehaviour extends SimpleBehaviour {
	...
	public void action() {
		...
		if (myPosition!=""){
			...
			if (agInfo.type == EntityType.AGENT_COLLECTOR && agInfo.reference != null) {
			...
			if (move.newTreasure || agInfo.goal == GoalType.shareInformation) {
				...
				if (tanker != null && tanker.lastUpdate > System.currentTimeMillis() - 2 * ((CustomAgent) myAgent).agentBlockingTime) {
					...
					String dest = "";
					try {
						dest = map.get(agInfo.reference).connectedNodes.get(0);
			...
			if (!MapUtils.checkPath(agInfo.path, map, agents, ((CustomAgent) this.myAgent).agentBlockingTime, myAgentName)) {
				try {
					...
					if (agInfo.goal == GoalType.explore) {
						...
						try {
							dest = MapUtils.getOldestNode(myPosition, map, agents, ((CustomAgent) this.myAgent).agentBlockingTime,
									myAgentName);
							agInfo.path = MapUtils.getPath(myPosition,dest, map, agents, ((CustomAgent) this.myAgent).agentBlockingTime,
									myAgentName);
						}
